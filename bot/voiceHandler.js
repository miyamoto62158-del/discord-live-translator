/**
 * voiceHandler.js - Discord ボイスチャンネルの音声受信・処理 (マルチモデル・各自DeepL・リアルタイムDiscordログ対応版)
 *
 * ユーザー別の音声ストリームを受信し、PCMに変換してバッファリング後、
 * WebSocketを通じて接続されているローカルPCクライアントへ転送する。
 * DeepL翻訳を実行し、ダッシュボードおよびDiscordテキストチャンネルのEmbedメッセージをリアルタイムで上書き更新します。
 */
...
// DiscordテキストチャネルへのリアルタイムEmbedフィード状態変数
...
/**
 * Discordのテキストチャンネルにリアルタイム翻訳ログを送信・上書き編集する
 */
async function updateDiscordTranslationFeed(username, originalText, translatedText, targetLang) {
  ...
  const tgtFlag = flagMap[targetLang.toUpperCase()] || '🌐';

  // ログエントリを作成
  const logEntry = `🎙️ **${username}**: ${originalText}\n${tgtFlag} **${targetLang}**: ${translatedText || "(翻訳不要)"}`;
  feedLogQueue.push(logEntry);

  // 直近6件に制限してEmbedが2000文字上限を超えないようにする
  if (feedLogQueue.length > 6) {
    feedLogQueue.shift();
  }

  // Embedを構築
  const embed = new EmbedBuilder()
    .setTitle("🌐 Live Translation Feed (リアルタイム翻訳ログ)")
    .setDescription(feedLogQueue.join("\n\n---\n\n"))
    .setColor("#5865F2")
    .setFooter({ text: "会話が流れるたびに、このメッセージがリアルタイムで自動更新されます ⏳" })
    .setTimestamp();

  try {
    if (!feedMessage) {
      // 初回は新規メッセージとして送信
      feedMessage = await feedChannel.send({ embeds: [embed] });
    } else {
      // 2回目以降は上書き編集
      await feedMessage.edit({ embeds: [embed] });
    }
  } catch (err) {
    console.error("❌ [Discord Feed] メッセージ送信・編集に失敗しました:", err.message);
    // メッセージが何らかの理由で削除されていた場合は新規作成にフォールバック
    try {
      feedMessage = await feedChannel.send({ embeds: [embed] });
    } catch (retryErr) {
      console.error("❌ [Discord Feed] 再送信にも失敗:", retryErr.message);
    }
  }
}
  ...
  ws.on("message", async (message) => {
    try {
      ...
      if (data.type === "register") {
        ...
        // 文字起こし結果を受信 → クラウド側でDeepL翻訳 → ダッシュボード＆Discord Feedへ配信
        ...
        // 2. DiscordテキストチャンネルへリアルタイムEmbed上書き送信
        updateDiscordTranslationFeed(data.username, originalText, tlResult.translated_text, currentTargetLang).catch(() => {});
      }
    } catch (err) {
...
function leaveChannel() {
  ...
  // DiscordテキストのLive Feed終了通知
  if (feedMessage) {
    const embed = new EmbedBuilder()
      .setTitle("👋 Live Translation Feed")
      .setDescription("❌ リアルタイム翻訳フィードは終了しました。")
      .setColor("#ED4245")
      .setTimestamp();
    feedMessage.edit({ embeds: [embed] }).catch(() => {});
  }
  feedChannel = null;
  feedMessage = null;
  feedLogQueue = [];
