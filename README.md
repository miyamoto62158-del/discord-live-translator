...
- 📝 **リアルタイム翻訳** — Gemini API (または DeepL) による超低遅延で自然なリアルタイム翻訳
- 👤 **ユーザー別表示** — 発言者ごとにアイコン・色分けして表示
- 🖥️ **Webダッシュボード** — ブラウザで結果を表示（画面共有にも最適）
- 💰 **完全無料** — Gemini API (および DeepL) の無料枠を活用した完全無料のクラウド翻訳システム
...
### 5. Gemini API キーの取得 (必須)

1. [Google AI Studio](https://aistudio.google.com/) にアクセスし、Googleアカウントでログインします。
2. **「Get API key」** をクリックして、新しいAPIキーを生成しコピーします（無料枠で十分に利用可能です）。

### 6. DeepL API キーの取得 (オプション)

※通常はGemini APIのみで翻訳が動作するため、DeepLの設定は不要です。DeepL翻訳エンジンを使いたい場合のみ設定してください。
1. https://www.deepl.com/pro-api にアクセス
2. **API Free** プランで登録（無料）
3. アカウント設定からAPIキーをコピーします。

### 7. 設定ファイルの編集

`bot/.env` ファイルを開いて、以下を記入します:

```
DISCORD_TOKEN=あなたのBotトークン
GEMINI_API_KEY=あなたのGemini APIキー
DEEPL_API_KEY=（オプション：DeepLを使う場合のみ記入）
```
...
### 「翻訳されない / 翻訳エラーになる」
- `bot/.env` の `GEMINI_API_KEY` が正しく入力されているか確認してください（無料枠の1日の上限等に達した場合は、時間をおいて再度お試しください）。
- （DeepLを使用している場合のみ）`bot/.env` の `DEEPL_API_KEY` やAPIの月間上限を確認してください。
