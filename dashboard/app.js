...
function handleMessage(data) {
    switch (data.type) {
        ...
        case 'transcription':
            ...
            const preview = activePreviews[data.user_id];
            if (preview) {
        ...
        case 'clear_preview':
            const previewToClear = activePreviews[data.user_id];
            if (previewToClear) {
                if (previewToClear.timeoutId) clearTimeout(previewToClear.timeoutId);
                if (previewToClear.element) previewToClear.element.remove();
                delete activePreviews[data.user_id];
            }
            break;
...
function updateTranscriptionPreview(data) {
    ...
    let previewState = activePreviews[userId];
    let card = previewState ? previewState.element : null;
    ...
    if (!card) {
        ...
        card.className = 'transcript-card preview';
        ...
        card.innerHTML = `
            ...
            <div class="card-content">
                <div class="card-header">
                    ...
                    <span class="card-timestamp">🎙️ 入力中...</span>
                </div>
                <div class="card-original">
                    <span class="card-lang-badge">🎙️ 認識中</span>
                    <span class="original-text text-preview-input"></span>
                </div>
                <div class="card-translation text-preview-translation-container">
                    <div class="translation-lang-row">
                        <span class="translation-to">🌐 翻訳中</span>
                    </div>
                    <div class="translation-text text-preview-translation"></div>
                </div>
            </div>
        `;

        // 確定メッセージより上に、しかしコンテナの最上部に挿入
        ...
        previewState = { element: card, timeoutId: null };
        activePreviews[userId] = previewState;
    }

    // 既存のタイムアウトタイマーがあればクリア
    if (previewState.timeoutId) {
        clearTimeout(previewState.timeoutId);
    }

    // 5秒間更新がなければプレビューカードを自動消去するタイマーを設定
    previewState.timeoutId = setTimeout(() => {
        if (previewState.element) {
            previewState.element.remove();
        }
        delete activePreviews[userId];
    }, 5000);

    // テキストを更新
    const inputEl = card.querySelector('.text-preview-input');
    const transEl = card.querySelector('.text-preview-translation');
    const transContainer = card.querySelector('.text-preview-translation-container');

    if (inputEl) inputEl.textContent = data.original_text || '...';
...
function clearTranscripts() {
    ...
    Object.keys(activePreviews).forEach(key => {
        const preview = activePreviews[key];
        if (preview.timeoutId) clearTimeout(preview.timeoutId);
        delete activePreviews[key];
    });
    messageCount = 0;
...
toggleCompactBtn?.addEventListener('click', toggleCompactMode);

// DeepL APIキー適用 (UI非表示に伴い無効化)
...
function toggleCompactMode() {
    ...
    localStorage.setItem('compact_mode', isCompact ? 'true' : 'false');
    updateCompactBtnText(isCompact);
}

function updateCompactBtnText(isCompact) {
    const btn = document.getElementById('toggle-compact-btn');
    if (btn) {
        btn.textContent = isCompact ? '↕️ 上部を広げる' : '↕️ 上部を縮める';
        btn.title = isCompact ? 'ヘッダーとメンバーパネルを表示します' : 'ヘッダーとメンバーパネルを非表示にして表示領域を広げます';
    }
}

// ── 起動 ──
const isCompactSaved = localStorage.getItem('compact_mode') === 'true';
if (isCompactSaved) {
    document.body.classList.add('compact-mode');
}
updateCompactBtnText(isCompactSaved);
...
function addDiscordChatCard(data) {
    ...
    const translationHtml = data.translated_text
        ...
        : `<div class="chat-translation skipped">
             <div class="chat-trans-text">(翻訳不要)</div>
           </div>`;
